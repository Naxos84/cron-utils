...
public class CompositeExecutionTime implements ExecutionTime {
    ...
    public CompositeExecutionTime(final List<ExecutionTime> executionTimes) {
    ...
    public Optional<ZonedDateTime> nextExecution(final ZonedDateTime date) {
        final Optional<Optional<ZonedDateTime>> next = executionTimes.parallelStream().map(e -> e.nextExecution(date)).filter(Optional::isPresent).sorted(
                (o1, o2) -> {
                    if (o1.isPresent() && o2.isPresent()) {
                        final ZonedDateTime first = o1.get();
                        final ZonedDateTime second = o2.get();
    ...
    public Optional<ZonedDateTime> lastExecution(final ZonedDateTime date) {
        final Optional<Optional<ZonedDateTime>> next = executionTimes.parallelStream().map(e -> e.lastExecution(date)).filter(Optional::isPresent).sorted(
                (o1, o2) -> {
                    if (o1.isPresent() && o2.isPresent()) {
                        final ZonedDateTime first = o1.get();
                        final ZonedDateTime second = o2.get();
